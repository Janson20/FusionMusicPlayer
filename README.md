...
* 服务端给的**推荐理由**直接显示在曲目行上（「你关注的音乐人新歌」「NO.43 飙升榜」
  「你可能喜欢的二次元歌曲」…），这也是它和「随便放一批歌」的区别
* **会自动续**：从漫游页开始播放后，队列剩不到 2 首时自动再取一批塞进队列，
  一直播下去；去听别的歌则自动交棒，不会往队列里偷塞歌
* 「换一批」换方向、「再多来点」往下接；推荐理由只在界面上显示，
  **不会跟着歌单 / 收藏落盘**（`Track.reason` 不进 `to_dict`）
...
* 专辑页：封面、专辑名、**可点的歌手**、发行日期与唱片公司、简介，加整张曲目列表
  （同样可一键播放全部 / 入队 / 收藏）
* 歌手与专辑互相可跳：专辑页点歌手名进歌手页，歌手页点专辑名进专辑页，
  返回键一层层退回来（`Esc` 也是先关最上面那层）
* 搜索的「全部」标签页置顶一张歌手卡片和一张歌单卡片（对应网易云的「综合」），
  点了直达歌手页 / 歌单页
* 其它音源（QQ / 酷我 / 酷狗 / 咪咕 / 本地文件）没有网易云的歌手 / 专辑 id，
  所以统一**按名字定位**；专辑还会拿**专辑名核对** id —— 别的平台的数字 id
  直接丢给网易云很可能撞出一张风马牛不相及的专辑
* 两个页面都只做「歌曲」这一页：专辑 / MV / 相似歌手 / 评论 / 点赞这些
  **没做**（各自要独立的接口与页面来承接）
